...
// Smart Suite access check — no stripe import needed on the client side
const LEGACY_PAID_PLANS = ["optimiser", "retirement", "optimiser_quarterly"];

function checkSmartETFAccess(sub: {
  apps?: string[];
  plan?: string;
  status?: string;
}): boolean {
  ...
  // New bundle system
  if (Array.isArray(sub.apps) && sub.apps.includes("smartetf")) return true;
  // Legacy: paid SmartSuper plans also grant access (shared Supabase project)
  if (sub.plan && LEGACY_PAID_PLANS.includes(sub.plan)) return true;
  return false;
}
...
export default function SmartETFDashboard() {
  ...
  async function loadProfile(userId: string) {
    try {
      ...
      // Smart Suite access check — inline, no stripe package needed
      if (subRes.data) {
        setIsSubscriber(checkSmartETFAccess(subRes.data));
      }
  ...
  async function saveProfile(updates: Partial<FinancialProfile>) {
    ...
    setProfile(next);
    if (!user) {
      localStorage.setItem("smartetf-profile", JSON.stringify(next));
      return;
    }
    try {
      ...
      if (next.portfolio?.holdings) {
        const { data: portData } = await supabase.from("portfolios").upsert({
          ...
          user_id: user.id,
          name: next.portfolio.name || "My Portfolio",
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#F7F8FA", fontFamily: "-apple-system,sans-serif" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 36, height: 36, border: "3px solid #E5E7EB", borderTopColor: "#1D9E75", borderRadius: "50%", margin: "0 auto 12px", animation: "spin 0.8s linear infinite" }} />
